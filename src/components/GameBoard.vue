<template>
  <div class="min-h-screen max-w-screen w-full transition-all duration-500 flex flex-col items-center p-2 sm:p-4 selection:bg-blue-500/30" :class="theme === 'dark' ? 'bg-zinc-950 text-zinc-100' : 'bg-zinc-50 text-zinc-900'">
    <header class="w-full max-w-2xl flex items-center justify-between mb-6 border-b pb-4 transition-colors duration-300" :class="theme === 'dark' ? 'border-zinc-800' : 'border-zinc-200'">
      ...
      <div class="flex items-center gap-2">
        <button
          @click="toggleSound"
          class="p-2.5 bg-zinc-500/10 hover:bg-zinc-500/20 rounded-xl transition-all active:scale-95 group text-zinc-500 hover:text-zinc-400 dark:hover:text-zinc-300"
          :aria-label="soundEnabled ? (currentLanguage === 'id' ? 'Matikan Suara' : 'Mute Sound') : (currentLanguage === 'id' ? 'Nyalakan Suara' : 'Unmute Sound')"
          :title="soundEnabled ? (currentLanguage === 'id' ? 'Suara Aktif' : 'Sound On') : (currentLanguage === 'id' ? 'Suara Mati' : 'Sound Muted')"
        >
          <!-- Sound ON Icon -->
          <svg v-if="soundEnabled" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-6 h-6 group-hover:scale-110 transition-transform">
            <path stroke-linecap="round" stroke-linejoin="round" d="M19.114 5.636a9 9 0 010 12.728M16.463 8.288a5.25 5.25 0 010 7.424M6.75 8.25l4.72-4.72a.75.75 0 011.28.53v15.88a.75.75 0 01-1.28.53l-4.72-4.72H4.51c-.88 0-1.704-.507-1.938-1.354A9.01 9.01 0 012.25 12c0-.83.112-1.633.322-2.396C2.806 8.756 3.63 8.25 4.51 8.25H6.75z" />
          </svg>
          <!-- Sound MUTED Icon -->
          <svg v-else xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-6 h-6 opacity-50 group-hover:opacity-100 transition-opacity">
            <path stroke-linecap="round" stroke-linejoin="round" d="M17.25 9.75L19.5 12m0 0l2.25 2.25M19.5 12l2.25-2.25M19.5 12l-2.25 2.25m-10.5-3.75l4.72-4.72a.75.75 0 011.28.53v15.88a.75.75 0 01-1.28.53l-4.72-4.72H4.51c-.88 0-1.704-.507-1.938-1.354A9.01 9.01 0 012.25 12c0-.83.112-1.633.322-2.396C2.806 8.756 3.63 8.25 4.51 8.25H6.75z" />
          </svg>
        </button>
    ...
    <!-- footer -->
    <div class="mt-3 flex items-center justify-center gap-3 text-xs sm:text-sm text-zinc-400">
      <span>
        Katla by <a href="//syofyanzuhad.dev" target="_blank" rel="noopener noreferrer" class="text-blue-400 hover:text-blue-300 transition duration-200">Syofyan Zuhad</a>
      </span>
      <span class="text-zinc-600">•</span>
      <a 
        href="https://github.com/syofyanzuhad/katla" 
        target="_blank" 
        rel="noopener noreferrer" 
        class="inline-flex items-center gap-1.5 text-zinc-400 hover:text-white transition duration-200" 
